...
const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
    open: { label: 'Abierto', color: 'text-yellow-700', bg: 'bg-yellow-50 border-yellow-200' },
    ...
    resolved: { label: 'Resuelto', color: 'text-green-700', bg: 'bg-green-50 border-green-200' },
    closed: { label: 'Cerrado', color: 'text-gray-700', bg: 'bg-gray-50 border-gray-200' },
};
                ...
                <div className="w-full max-w-md">
                    ...
                    <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 shadow-2xl">
                        ...
                        <p className="text-center text-white/40 text-xs mt-6">
                            Usá el email registrado de tu empresa en Zeron CRM
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div className="px-6 py-5 border-b border-gray-100">
                            ...
                            <p className="text-xs text-gray-400 mt-2">
                                Creado: {new Date(selectedTicket.created_at).toLocaleString('es-AR')}
                                {selectedTicket.closed_at && ` · Cerrado: ${new Date(selectedTicket.closed_at).toLocaleString('es-AR')}`}
                            </p>
                        </div>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
                <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all"
                    >
                        ...
                        {tickets.map(ticket => {
                            ...
                            return (
                                <button
                                    ...
                                    className="w-full bg-white rounded-xl border border-gray-200 hover:border-blue-200 hover:shadow-md transition-all p-4 text-left group"
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-xs font-mono text-gray-400">{ticket.ticket_number}</span>
                                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${st.bg} ${st.color}`}>{st.label}</span>
                                                <span className={`text-[10px] font-medium ${pr.color}`}>{pr.label}</span>
                                            </div>
                                            <h3 className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors truncate">{ticket.subject}</h3>
                                            <p className="text-xs text-gray-400 mt-1">
                                                {new Date(ticket.created_at).toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: 'numeric' })}
                                            </p>
                                        </div>
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" className="shrink-0 mt-2 group-hover:stroke-blue-400 transition-colors"><polyline points="9 18 15 12 9 6"/></svg>
                                    </div>
                                </button>
                            );
                        })}
